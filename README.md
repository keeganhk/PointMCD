...
### Usage
**[Datasets]** Download our [pre-processed datasets](https://pan.baidu.com/s/1olfgmM9dDLRVuiVe6P6iZw?pwd=lcra) (password: lcra) and put them under the ```data``` folder. Or you can also use our [pre-processing code](https://pan.baidu.com/s/1-7Z8Uk79uVC4UtO_rhcGNA?pwd=33tv) (password: 33tv) to render multi-view images and the corresponding point-wise visibility for your own data.

**[Scripts]** We provided the training scripts of different teacher networks and released their corresponding pre-trained model parameters under the ```ckpt/teacher``` folder. The teacher knowledge information is exported in advance, which can be downloaded from [here](https://pan.baidu.com/s/1q0-Q7y323TwWrNEj16nLEA?pwd=0f9f) (password: 0f9f), and then put under the ```expt``` folder. The trained student models will be stored under the ```ckpt/student``` folder.
