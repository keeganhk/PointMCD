...
ckpt_params = os.path.join(ckpt_root, 'student', 'PointNet_Seg_Distiller_ShapeNetPart.pth')
...
net = PointNet_Seg_Distiller().cuda()
max_lr = 1e-1
min_lr = 1e-3
train_epoch = 200
...
for epc in range(1, train_epoch+1):
    ...
    for (points, labels, mvv, class_id, mv_cdw_t, model_name) in tqdm(tr_loader):
        ...
        loss_task = smooth_cross_entropy(logits.view(batch_size*num_points, num_parts), labels.view(batch_size*num_points), 0.05)
        loss_dist = criterion(mv_cdw_s, mv_cdw_t)
        loss = 0.1*loss_task + 1.0*loss_dist
        loss.backward()
        optimizer.step()
