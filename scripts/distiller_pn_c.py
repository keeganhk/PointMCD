...
ckpt_root = '../ckpt'
expt_root = '../expt'
ckpt_params = os.path.join(ckpt_root, 'student', 'PointNet_Cls_Distiller_ModelNet40.pth')
...
net = PointNet_Cls_Distiller().cuda()
max_lr = 1e-1
min_lr = 1e-3
train_epoch = 500
...
for epc in range(1, train_epoch+1):
    ...
    for (pts, mvv, cid, cdw_t, mv_cdw_t, lgt_t, mv_lgt_t, model_name) in tqdm(tr_loader):
        ...
        optimizer.zero_grad()
        cdw_s, mv_cdw_s, lgt_s, mv_lgt_s, loss_rglz = net(pts, mvv)
        loss_task = smooth_cross_entropy(lgt_s, cid)
        loss_dist_cdw_mv = criterion(mv_cdw_s, mv_cdw_t)
        loss = 0.1*loss_task + 0.001*loss_rglz + 1.0*loss_dist_cdw_mv
        loss.backward()
        optimizer.step()
    ...
    for (pts, cid, model_name) in tqdm(te_loader):    
        with torch.no_grad():
            ...
            cdw_s, mv_cdw_s, lgt_s, mv_lgt_s, loss_rglz = net(pts, placeholder_mvv)
            prd = lgt_s.max(dim=-1)[1]
            num_correct += (prd==cid).sum().item()
            num_samples += bs
